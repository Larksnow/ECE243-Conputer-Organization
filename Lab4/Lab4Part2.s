...
CONFIG_KEYS:                                    
              LDR			R0, =0xFF200050
			  MOV			R1, #0xF
			  STR			R1, [R0, #0x8] //clear edge capture
			  MOV 			PC, LR
...
IRQ_HANDLER:
				...
				CMP			R5, #73 //determine which interrupt happened
				BEQ			KEY_ISR
				CMP			R5, #29
				BEQ			PRIV_TIMER_ISR
...
KEY_ISR:        
				...
				LDR		R1, RUN
				EOR		R1,#1
				STR 	R1, RUN //revert RUN each time
				LDR		R2, COUNT
				ADD		R2, R1
...
PRIV_TIMER_ISR:
                LDR     R0, =0xFFFEC600
				MOV     R3,#1
				STR     R3,[R0,#12] //reset interrupt status first
				LDR     R2,COUNT
				LDR		R0,RUN
				ADD     R2,R0
				...
				LDR		R3, =0b1111111111
				CMP     R2,R3 //clear when all LEDs are on
				BGT		CLEAR
				STR		R2,COUNT
				B		TIMER_END
