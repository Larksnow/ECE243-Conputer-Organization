...
CONFIG_PRIV_TIMER:                             
                LDR			R0, =0xFFFEC600
				LDR			R3, SPEED //different SPEED has different load value
				STR			R3, [R0]
				MOV			R3, #7 //set A,E,I bit in control registor 
				STR			R3, [R0, #8] //A,E,I are all 1
				MOV         R3, #1
				STR         R3, [R0,#12] //reset interrupt status
				MOV			PC, LR
...
CONFIG_KEYS:                                    
              LDR			R0, =0xFF200050
			  MOV			R1, #0xF
			  STR			R1, [R0, #0x8] //reset edge capture
			  MOV 			PC, LR
...
IRQ_HANDLER:
                PUSH		{R0-R7,LR}
				LDR			R4, =0xFFFEC100
				LDR			R5, [R4, #0x0c] //read interrupt id
				
				CMP			R5, #73 //determine where to go
				BEQ			KEY_ISR
				CMP			R5, #29
				BEQ			PRIV_TIMER_ISR
...
KEY_ISR:        
				...
				CMP 	R1, #0x1 //identify which key is pressed
				BEQ		CHECKKEY0
				CMP		R1, #0x2
				BEQ 	CHECKKEY1
				CMP		R1, #0x4
...
CHECKKEY0:		LDR		R1, RUN //stop/restart 
				EOR		R1,#1
				STR 	R1, RUN
				LDR		R2, COUNT
				ADD		R2, R1
...
PRIV_TIMER_ISR:
				...
				LDR		R3, =0b1111111111 //reset when all LEDs are on 
				CMP     R2,R3
				BGT		CLEAR
				STR		R2,COUNT
				B		TIMER_END
